...
| **vertical pitch** (footer switch, a test) | draws the PITCH strip upright on every tab: heel at the bottom, toe at the top, the ▲ marker moving up and down, the value running left → right with its labels along the bottom; chips stack up the right side. Everything else (zones, points, popovers) works the same, just turned (v1.21) |
